/*
  ...
  modified by A.J <andrwj@gmail.com>
*/
;(function(){
      ...
      for (var name in props) {
        if (name == 'include') {
          if (props[name] instanceof Array) {
            for (var i = 0, len = props[name].length; i < len; ++i) Class.include(props[name][i]);
          } else {
            Class.include(props[name]);
          }
        } else if (name == 'extend') {
          if (props[name] instanceof Array) {
            for (var i = 0, len = props[name].length; i < len; ++i) extend(props[name][i]);
          }
          else {
            extend(props[name]);
          }
